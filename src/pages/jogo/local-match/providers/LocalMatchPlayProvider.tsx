...
import { resolvePokemonForMatch, resolveMatchDexMap, mergeDexRecords } from '../../../../lib/game/matchPokemonResolve';
import {
...
export function LocalMatchPlayProvider({ hostName, children }: LocalMatchPlayProviderProps) {
  ...
  useEffect(() => {
    if (!localMatch.clientState || Object.keys(localMatch.pokemonByDex).length === 0) return;
    if (localMatch.phase !== 'playing') return;
    void applyDexForView(localMatch.clientState, viewerSide);
  }, [localMatch.clientState, localMatch.phase, localMatch.pokemonByDex, applyDexForView, viewerSide]);
