  ...
  if (exactMatch) {
    ...
    if (next.finalResponseFor === playerSide) {
      if (hasPlayerCompleted(next, playerSide)) {
        ...
        outcome = 'KEEP_TURN';
        message = 'Correct guess in the final round; keep playing.';
      }
      ...
      message = 'Opponent gets one extra round to try for a tie.';
    } else {
      outcome = 'KEEP_TURN';
